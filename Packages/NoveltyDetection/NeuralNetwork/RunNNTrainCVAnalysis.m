...
load(sprintf('%s/TrainInformationNN_novelty_%s.mat',outputpath,class_labels{1}));
...
for novelty_class = 1:numel(class_labels) % todas as classes
    ...
    for topo = 1:length(possible_topo)
        ...
        for ifolds = 1:n_folds
            ...
            load(sprintf('%s/mat/RunNNTrainCV_novelty_%s_topo_%02i_fold_%02i.mat',outputpath,class_labels{novelty_class},possible_topo(topo),ifolds));
                ...
                if SP > sp
                    ...
                    for i_thr = 1:size(possible_thr,2)
                        for iclass = 1:numel(class_labels)-1 % todas as classes
                            ...
                            v_eff(novelty_class,topo,i_thr,ifolds,iclass) = sum(a_test >= possible_thr(i_thr) & target(iclass,:)== 1 & id==iclass)/sum(target(iclass,:)== 1);
                        end
...
for topo = 1:length(possible_topo)
    for novelty_class = 1:numel(class_labels)
        ...
        saveas(h,sprintf('%s/pict/RunNNTrainCVAnalysis_topo_%i_novelty_%s_class_eff_per_thr.png',outputpath,possible_topo(topo),class_labels{novelty_class}));
        close(h);
        clear('H');
...
for novelty_class = 1:numel(class_labels)
    h = figure;
    load(sprintf('%s/mat/RunNNTrainCV_novelty_%s_topo_%02i_fold_%02i.mat',outputpath,class_labels{novelty_class},possible_topo(choose_topo),choose_fold));
    load(sprintf('%s/TrainInformationNN_novelty_%s.mat',outputpath,class_labels{novelty_class}));
    ...
    for iclass_target = 1:dim_target
        for iclass_output = 1:dim_output
            ...
            [y,x]=hist(output_plus_novelty(iclass_output,target_plus_novelty(iclass_target,:)==1),bin_centers);
    ...
    print(h,sprintf('%s/pict/RunNNTrainCVAnalysis_topo_%i_novelty_%s_class_eff_per_thr.png',outputpath,possible_topo(choose_topo),class_labels{novelty_class}),'-dpng','-r0');
    close(h);
