...
% Train Information
if(~exist(sprintf('%s/TrainInformationNN.mat',outputpath),'file'))
    error('DO %s/DataHandler/RunNNTrainCVAnalysis\n',workspace);
else
    load(sprintf('%s/TrainInformationNN.mat',outputpath));
end
...
for topo = 1:length(possible_topo)
    ...
    for ifolds = 1:n_folds
        ...
        for i_init = 1:n_init
            ...
            load(sprintf('%s/mat/RunNNTrainCV_topo_%02i_fold_%02i.mat',outputpath,topo,ifolds))
